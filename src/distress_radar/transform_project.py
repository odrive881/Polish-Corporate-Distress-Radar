...
them into asset checks) instead of a single pass/fail.
...
def declared_audits(project: Path = TRANSFORM_DIR, gateway: str = "test") -> dict[str, list[str]]:
...
def build_dq_models(project: Path = TRANSFORM_DIR, gateway: str = "local") -> DqBuild:
    ...
    context = Context(paths=project, gateway=gateway)
    try:
        return DqBuild(audits=_audit(context), row_counts=_row_counts(context))
    finally:
        context.close()


def _audit(context: Context) -> tuple[AuditOutcome, ...]:
...
def _row_counts(context: Context) -> dict[str, int]:
    counts: dict[str, int] = {}
    for name in DQ_MODELS:
