...
The official XSDs for every structure the Phase 1 seed uses were downloaded, with everything they import, and vendored under `config/xsd/` (51 files; `catalog.yaml` records each URL and SHA-256 — 53 after the two CRWDE templates added in the second addendum §5). What they settle:
...
need their own overrides for the three lines above**, and that test is the acceptance criterion. Done in plan
0005 step D: `small-2025-v1-3` carries them, and the three lines map onto the existing `.R2025` codes
(`IS.CALC.A`, `IS.CALC.B`, `IS.COMP.B.VIII`) — no narrowing codes of its own were needed.
...
| `13821` | 2025-08-12 | `JednostkaMikro` | `SFJMIZ (2)` — mapped (`micro-2025-w2-v1-0`, plan 0005 step D) |
...
### 6. What was built on this (plan 0005 steps C–F, close-out 2026-09-21)

The classification above became 46 new chart codes (554 → 600), three new bodies (`jednostka_mala`,
`jednostka_mikro_v1_2`, `jednostka_mikro_v1_3`) and seven specs. Two findings from the build belong with the
research, because neither is visible in the schemas alone:

- **Codes are matched by meaning, never by position.** The section letters do not line up between the forms:
  the small form has no operating-result line, so its `F` is the full form's `G` and the shift continues to
  the end of the statement. A path- or position-based reuse would have mapped "Przychody finansowe" onto
  "Pozostałe koszty operacyjne", and every identity check would have passed.
- **A `JednostkaMala` filing need not contain the small-form statements.** The envelope accepts either the
  small or the full body, chosen per statement — 15 of the 30 small-form seed files use the full one, and 3
  mix the two. `kodSystemowy` and `wersjaSchemy` are identical across all three shapes, so detection cannot
  separate them; each spec therefore lists body alternatives and the engine picks the one the document
  contains. Recorded here because ADR 0005's §6C1 rule ("detect from the header") remains true for the
  *version* and is not sufficient for the *body*.
