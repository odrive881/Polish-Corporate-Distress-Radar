...
| What's the plan for the next build step? | `docs/plans/` (numbered; the highest number not marked draft or deferred = current) |
...
6. **Legal entities only.** Never ingest or store natural persons, including consumer bankruptcies in insolvency registers. Filter at acquisition. Filed documents, RDF details and KRS extracts go through `acquisition/redaction.py` before storing, MSiG notices are stored only as person-free records (`acquisition/msig_client.py`), and test fixtures must not contain real signatures or names (ADR 0009). A blocking `personal_data` asset check scans the whole raw store after every A3 run (plan 0011).
...
| **MSiG** | Monitor Sądowy i Gospodarczy — the court and economic gazette; its notice base (from 2001) is searchable by KRS as JSON |
...
- **Update, 2026-09-23 — Phase 4 sources (plan 0008, ADR 0011):** the **open KRS API** (`api-krs.ms.gov.pl`, `OdpisPelny`, no key, no WAF) returns the full extract with history and records proceedings in both eras. It is the primary legal-event source, redacted of natural persons before hashing (`krs-json-2`, ADR 0009 addendum). Every response carries its own timestamp, so unchanged extracts are recognised by fingerprint, not cache. **MSiG** (`wyszukiwarka-msig.ms.gov.pl/api`) is a JSON search by KRS with notice text, back to 2001. Its text names people and is never stored: notices are reduced at fetch time to person-free records (`msig_client.py`) and typed by `config/mappings/msig_notice_kinds.yaml`. **KRZ** sits behind the same Imperva WAF as RDF and is not built. The route there is manual capture or a sanctioned channel, never a bypass. Traps: KRS labels a post-2016 sanacja "POSTĘPOWANIE NAPRAWCZE"; its date field `dataNadaniaKlauzuliWykonalnosci` is the decision date; registry entries lag decisions by up to 21 months. So near the cutoff, `alive` labels can miss an event not yet entered (plan 0008 status, open risk).
