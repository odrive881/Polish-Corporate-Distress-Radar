...
- **Update, 2026-09-14 — probed empirically (`docs/adr/0007-rdf-access-probe-results.md`, status proposed):** `ekrs.ms.gov.pl/rdf/rd/` now redirects to an info page; the live search (`rdf-przegladarka.ms.gov.pl`) and `rdf.ms.gov.pl` sit entirely behind an Imperva Incapsula WAF that returns HTTP 200 block pages to automated clients. Plain httpx does not work, and passing the challenge with a browser is a terms-of-use decision, not a routing one. **A3 / plan 0003 is on hold pending that decision** — don't build an automated bypass.
