...
WITH e2_files AS (
  ...
  SELECT
    ...
    entity_key,
    CASE WHEN stage IN ('A1', 'A2', 'A3', 'A4', 'C4') THEN NULL ELSE source_document_hash END AS file_hash,
...
LEFT JOIN first_detected AS d
  ...
  AND d.file_hash IS NOT DISTINCT FROM CASE
    WHEN s.stage IN ('A1', 'A2', 'A3', 'A4', 'C4') THEN NULL
