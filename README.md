...
**Status:** Phase 2 complete (`docs/plans/0004-phase-2-canonical-parsing-identities.md`, close-out 2026-09-17). The full-form Ministry of Finance statements (schemas 1-0, 1-2 and 1-3, the 2025 CRWDE wariant 2, and the thousands-of-złoty twin) are parsed from the stored downloads into `financial_statements_canonical`: Parquet under `WAREHOUSE_DIR` (ADR 0008), with full lineage and a `quality_grade` from the accounting identity checks, which run as Dagster asset checks. `restatement_events` holds prior-year differences. For the 17-entity seed that is 88 statements across 13 entities: 49 pass, 16 warn, 23 quarantined for genuine filing defects. Small- and micro-form structures (42 seed statements, and the only 4 entities with no data yet) and the PDF tier are the rest of Phase 3 — plans 0005 to 0007; plan 0005 step A (full-form schema 1-3) is done. Phase 1 (acquisition) is complete; RDF documents are captured by hand as HAR files (§ "Manual RDF capture") because RDF serves automated browsers a CAPTCHA (ADR 0007).
