...
**Status:** Phase 4 complete (`docs/plans/0008-phase-4-legal-events-outcome-labels.md`, close-out 2026-09-23). The seed's legal timeline is built from the **full KRS extract** (open KRS API; both eras, entry-dated) and **MSiG notices** (JSON search, back to 2001). Both are stored only in redacted or person-free form (ADR 0009 addendum). They give 89 `legal_events` rows in 70 deduplicated events (registrations included), and **all 9 seed entities with a distress hint are found independently**, with dates and source documents; the other 8 have none. Outcome labels are built in SQLMesh: 12 and 24 months, censored at each entity's cutoff, regime and source-era flags. They are frozen by content hash: the current label set is `a1ac9fb07f79…`, 4,694 rows, reproduced exactly on rebuild. The earlier set, `a5da757f8341…`, stays frozen, from before the Phase 4 review fixes. KRZ is not built: it sits behind a WAF (ADR 0011). Phase 3 (the canonical financial model, 129 statements across all 17 entities, `quarantine` and `dq_mart` in SQLMesh) and Phase 1 (acquisition; RDF captured by hand as HAR files, ADR 0007) are complete before it. Next: Phase 5, the feature store.
...
`seed_acceptance` on `legal_events` checks every seed entity against its status hint. A model trains on a `label_set_hash`, never on "the latest labels". Label parameters live in `config/labels/`; the procedure taxonomy in `config/statutory/procedure_taxonomy.yaml`; MSiG notice typing in `config/mappings/msig_notice_kinds.yaml`. Editing the MSiG vocabulary (`msig_vocabulary.yaml`) re-fetches every notice on the next run, because the text is not kept.
